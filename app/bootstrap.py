...
A £150 deposit secures your date."""),
    ...
    template_rows = {
        ...
        "review_request": ("After-wedding review request", "A little favour after your wedding", "Hi {client_first_name},\n\nI hope you are both still on a high after your wedding and enjoying looking back through your photographs and films. It was genuinely lovely being part of your day.\n\nIf you have a spare couple of minutes, would you mind leaving Weddings By Mark a Google review? Reviews make a huge difference to a small business like mine and help future couples know what to expect. The easiest way is to search for Weddings By Mark on Google and press Write a review.\n\nThere is absolutely no pressure, but I would be incredibly grateful.\n\nThank you again for trusting me with such an important day.\n\nMark\nWeddings By Mark\n{business_phone}"),
        ...
        "quote_accepted_admin": ("Quote accepted - notify Mark", "Quote accepted - {couple_or_company} - {event_date}", "Hi Mark,\n\n{couple_or_company} have accepted their Weddings By Mark quote.\n\nWedding date: {event_date}\nVenue/location: {venue_or_project}\nPackage: {accepted_package}\nAdditional items: {accepted_addons}\nInvoice: {invoice_number}\nQuote total: {quoted_total}\nBooking fee to look out for: {deposit_amount}\n\nTheir date remains provisional until you receive and record their first payment. This is your heads-up to look out for the transfer and confirm it promptly when it arrives.\n\n[OPEN THEIR BOOKING & PAYMENTS]({admin_url})\n\nYou can reply directly to this notification and it will go to the couple.\n\nWeddings By Mark Booking System"),
        "booking_form_submitted_admin": ("Wedding Booking Form submitted - notify Mark", "Wedding Booking Form {submission_action} - {couple_or_company}", "Hi Mark,\n\n{couple_or_company} have {submission_action} their {form_name}.\n\nWedding date: {event_date}\nVenue/location: {venue_or_project}\nPackage: {package_name}\n\nTheir answers are now ready for you to review in the booking system.\n\n[REVIEW THEIR WEDDING BOOKING FORM]({admin_url})\n\nYou can reply directly to this notification and it will go to the couple.\n\nWeddings By Mark Booking System"),
        "final_timings_submitted_admin": ("Final Wedding Timings submitted - notify Mark", "Final Wedding Timings {submission_action} - {couple_or_company}", "Hi Mark,\n\n{couple_or_company} have {submission_action} their {form_name}.\n\nWedding date: {event_date}\nVenue/location: {venue_or_project}\n\nTheir latest timings are ready for you to review and download as a PDF.\n\n[REVIEW THEIR FINAL WEDDING TIMINGS]({admin_url})\n\nYou can reply directly to this notification and it will go to the couple.\n\nWeddings By Mark Booking System"),
        "contract_signed_admin": ("Wedding agreement signed - notify Mark", "Wedding agreement signed - {couple_or_company} - {event_date}", "Hi Mark,\n\n{couple_or_company}'s wedding agreement has now been signed.\n\nAccepted by: {accepted_name}\nAgreement version: {contract_version}\nWedding date: {event_date}\nVenue/location: {venue_or_project}\n\nThe completed agreement is protected in their booking and ready for you to view or download.\n\n[OPEN THEIR SIGNED AGREEMENT]({admin_url})\n\nYou can reply directly to this notification and it will go to the couple.\n\nWeddings By Mark Booking System"),
    ...
    for brand in (Brand.WBM, Brand.IVORY):
        for key, (name, subject, body) in template_rows.items():
            if brand == Brand.IVORY and key in ("final_questionnaire", "enquiry_received", "quote_accepted",
                                                ...
                                                "quote_followup_final", "deposit_due_1", "check_in_120", "check_in_30",
            ...
            if not existing_template and (fresh_install or key in (
                ...
                old_paragraph = (
                    ...
                    "know, simply reply to this email."
                )
                invitation = (
                    "Please complete your Final Wedding Timings Form using your secure booking "
                    "link below. It brings your venues, preparations, key timings and wedding-day "
    ...
    # check_in_30 invitation; the old template must never be offered or sent.
...
Once you have accepted your quote, please use **{payment_reference}** as your bank-transfer payment reference.
...
Weddings By Mark"""
    ...
    if (quote_template and quote_template.display_name == "Initial package quote"
            and "use the invoice number shown in your wedding account as the payment reference"
            in quote_template.body):
        quote_template.body = quote_template.body.replace(
            "use the invoice number shown in your wedding account as the payment reference",
            "use **{payment_reference}** as your bank-transfer payment reference",
        )
    ...
    if (accepted_template and accepted_template.display_name == "Package accepted and invoice ready"
        ...
        accepted_template.body = template_rows["quote_accepted"][2]
    if (accepted_template and accepted_template.display_name == "Package accepted and invoice ready"
            and "{payment_reference}" not in accepted_template.body):
    ...
    for reminder_key in ("deposit_due_1", "balance_due_7", "balance_due_1", "balance_overdue_2"):
        reminder_template = db.scalar(select(EmailTemplate).where(
            EmailTemplate.brand == Brand.WBM,
            EmailTemplate.template_key == reminder_key,
        ))
        if (reminder_template and reminder_template.display_name == template_rows[reminder_key][0]
                and "{payment_reference}" not in reminder_template.body):
            reminder_template.body = reminder_template.body.replace(
                "You can view your invoice, bank details and payment reference here:",
                "Your bank-transfer payment reference is **{payment_reference}**.\n\nYou can view your invoice and bank details here:",
            )
